...
#From Github's '30 seconds of code'.
...
def acomp(x,y,moon):
    ...
    #parameters of gravity field
        ...
        g = a/9.81964973772
        return(a,ax,ay,g,dearth,dmoon)

#Check to see if the coordinates of the object are within the earth
#according a radius of 6,371,000 meters.
#Added arithmetic comparison to avoid unnecessary calculations every time.
#Made obsolete by taking already calculated distance from earth and moon
#in the acomp method to avoid doubly calculating. This also lets me
#check if it hits the moon.
def Inside(x,y):
    if satellite_A[1] < 6371000 or satellite_A[2] < 6371000:
...
def work():
    ...
    theta = degrees_to_radians(float(input('Angle (º): ')))
    ...
    ##################calculate movement using with Euler's method##################
    ...
    while count < time:
        ...
        #the planet.
        if accsuite[4] <= 6371000:
            print("Broke (earth)")
            error = True
            break
        elif accsuite[5] <= 1737100:
            print("Broke (moon)")
            error = True
            break
    ...
    #Done with program.
    playsound('siren.mp3')
    return satellite_A, velocity, count, moon, error
